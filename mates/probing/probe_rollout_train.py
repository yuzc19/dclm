...
def main(args):
    ...
    # remember to use 01_1 to train random baseline
    args.resume = "gs://cmu-gpucloud-zichunyu/out/dclm_logs/baseline_01_1_fasttext-d=1024_l=24_h=8-warm=2000-lr=0p003-wd=0p033-cd=3e-05-bs=512-mult=4-seed=124-tokens=32929300480/checkpoints/epoch_2.pt"
    # args.resume = "gs://cmu-gpucloud-zichunyu/out/dclm_logs/baseline_01_1_fasttext-open_lm_1b_swiglutorch-warm=5000-lr=0p003-wd=0p033-cd=3e-05-bs=256-mult=1-seed=124-tokens=28795904000/checkpoints/epoch_1.pt"
    ...
    # probe_data = "gs://cmu-gpucloud-zichunyu/data/refinedweb_01_0/fasttext/fasttext_filter/processed_data/pythia_tokenized/train.pt"
    ...
    bootstrap_indices = np.load("/project/flame/zichunyu/out/oracle/pythia-410m/epoch_2/rollout-dim-flan-r10-bootstrap.npy")
    ocache = f"baseline_01_1_fasttext-d=1024_l=24_h=8-warm=2000-lr=0p003-wd=0p033-cd=3e-05-bs=512-mult=4-seed=124-tokens=32929300480/checkpoints/epoch_2/oracle/{seed}"
    oracle = []
    ...
    for i in tqdm(range(left, right)):
        ...
        # for probe_index in range(0, len(probe_indices), 512):
        #     for j in range(0, 512, 16):
        #         probe_data = torch.cat(
        #             [
        #                 train_dataset[i]
        #                 for i in probe_indices[probe_index + j : probe_index + j + 16]
        #             ]
        #         )
        #         train(model, optimizer, probe_data, accumulation=(j != 496))
        #     # input_ids += probe_data[0][:-1].cpu().numpy().tolist()
        #     scores.append(evaluate(model, val_dataloader)[0] - eval_base)
